...
const highlight = (t, source, expect) => {
	t.is(h(source), expect, `${inspect(source)} should be highlighted as ${inspect(expect)}`);
};
...
test('keyword', t => {
	w`break catch class const continue default do else finally function for from
		...
		export extends in instanceof of new return throw typeof void yield`
		.forEach(keyword => {
...
test('number', t => {
	w`3 3.14 3.14e10 3.14e-10 0x0123456789abcdefABCDEF 0b0101 0o1234 01234 42. .42`
		.forEach(source => {
